...
- **`pm:street-takeaways`** — per-ticker log of parsed FactSet "SA: Street Takeaways" alert emails (`{ [TICKER]: StreetTakeaway[] }`, newest first, capped at 5/ticker). Each entry holds the post-earnings analyst roundup: per-firm price-target changes with valuation basis + distilled argument, full-panel rating mix, average target and its change, valuation vs the name's own 5-year history, and consensus estimate revisions. Ingested via the Gmail inbox — these are BODY-TEXT emails (no attachment), matched by sender (`FactSet_Alerts@factset.com`) or the "Street Takeaways" subject, parsed by a small Anthropic call in `app/lib/street-takeaways-parse.ts`, and scoped to Portfolio/Watchlist names (others skipped). Deduped on (ticker, date, event). Feeds the scoring prompt (catalysts / researchCoverage / historicalValuation) and the stock page's Street Takeaways tile. Read-only w.r.t. `pm:stocks` — it NEVER writes a score. Losing it costs the ingested analyst context until the next alerts arrive.
