...
- **`pm:sia-history`** — **append-only** per-ticker log of SIA relative-strength readings. Shape: `{ [TICKER]: SiaHistoryEntry[] }` where each entry is `{ date, timestamp, smax?, percentile?, rank?, universeSize? }`. Written on every SIA CSV ingest (emailed path via `logSiaHistory` in `inbox-dispatch`, manual path via `POST /api/kv/sia-history`), scoped to **held names only** (Portfolio + Watchlist) — logging all 566 universe rows weekly would grow the key without serving the question it exists for. Same date invariant as `pm:portfolio-snapshots`: one entry per ticker per day, today only, and the FIRST reading of a day wins so a re-upload or an out-of-order batch can never rewrite history. Capped at 220 entries/ticker (~4 years weekly). Exists because `stock.sia` holds only the latest SMAX, and SMAX is a 0-10 integer too coarse to show drift — a name can slide from the 91st to the 62nd percentile and still read "10". Read by `computeChangeEvents` to raise the `relative-strength` event (`THRESHOLDS.siaPercentileDrop`). **Monitoring only — it NEVER feeds a score**; `mapSmaxToRelativeStrength` remains the sole path from SIA into the composite. **BACKED UP** (deliberately not in the backup exclude list) — losing it loses the trend evidence.
- **`pm:regime-history`** — **append-only** daily log of the regime engine's read (`app/lib/regime-history.ts`): `{ rows: RegimeHistoryRow[] }` ascending by Eastern date, each row = `{ date, computedAt, rawLabel, label, weightedScore, score100, riskOn, riskOff, total, signals: {name→direction}, horizons: {tactical,cyclical,structural}, spx, vix }`. Written ONLY by `computeFromYahoo()` in `market-regime-refresh.ts` (the 30-min route recompute + nightly cron); one row per weekday, today only, same-day recomputes REPLACE today's row, past rows never touched, capped at 1500. Two jobs: (1) **hysteresis** — `composite.label` only follows `composite.rawLabel` after `HYSTERESIS_CONFIRM_DAYS` (3) consecutive sessions read the same way, so one bar can't flip the canonical label (the pending flip is exposed as `composite.pending`); (2) **validation** — `spx` per row makes "did Risk-On precede better forward returns?" answerable from our own data, the prerequisite to changing any signal/weight/threshold with evidence. `GET /api/kv/regime-history` is read-only; there is deliberately no client write path. **BACKED UP** (not in the exclude list) — losing it loses the evidence base and resets hysteresis (harmless: the next refresh commits the raw label).
- **`pm:market-drivers`** — cached output of `/api/market-drivers` (`app/lib/market-drivers.ts`): cap-weighted contribution of every S&P 500 + TSX 60 constituent (lists in `factor-constituents.ts`) to its index's 1d/1w return, per-sector rollups with leaders/laggards, and 1d/1w/1m/3m returns for the sector + industry ETF universe — one batched FactSet cross-sectional pull (`P_TOTAL_RETURNC(-1D,0)` etc. + `FG_MKT_VALUE` + `FG_GICS_SECTOR`). Feeds the Brief summary's "Market drivers" zone and the regime sector map. Pure regenerable cache — rebuilt by the nightly backup cron and lazily on GET when older than 20h (`?refresh=1` forces); on a failed rebuild the stale cache is served with `error` set, and an empty build never overwrites a good snapshot. Safe to nuke.
- **`pm:earnings-dates`** — 24h cache of Yahoo next-earnings dates for tickers NOT in `pm:stocks` (the Suggested watchlist names the Brief calendar shows). `{ [TICKER]: { date, fetchedAt } }`, read-merge-write. Pure cache. Safe to nuke.
- **`pm:action-state`** — the PM's done / snoozed marks on the Brief action queue: `{ [actionId]: { status: "done"|"snoozed", at, until? } }`. Managed by `/api/kv/action-state` (GET, POST one entry, read-merge-write, entries older than 45 days pruned on write). Action ids are date-scoped or event-scoped, so a stale mark can never hide a new item. Losing it only re-surfaces already-handled items.
- **`pm:market-regime`** — cached output of `/api/market-regime` (Yahoo-derived regime snapshot: SPX 10M trend, RSP/SPY breadth, sector ratios XLY/XLP & XLK/XLU & MTUM/USMV, cross-asset VIX/DXY/^TNX/CL=F, global ^STOXX & ^N225, plus composite Risk-On/Neutral/Risk-Off label). Shape: a single JSON blob matching `MarketRegimeData` in `app/lib/market-regime.ts`. Since Sept 2026 the composite is a FIXED slate (every signal votes, neutral included — never let a neutral signal drop out of the denominator again, that is what made the label flip on quiet days), rolled up as the horizon-weighted score from `horizons.ts` (`composite.weightedScore` / `score100`), with `composite.label` hysteresis-smoothed via `pm:regime-history` and `composite.rawLabel` the unsmoothed read. FRED-derived votes (ISM / credit / curve) fall back to the cached readout when a fetch fails so the slate stays fixed. Pure cache (no user input) — recomputed on GET when older than 30 minutes or when `?refresh=1` is passed. On Yahoo failure the route falls back to the cached value (even if stale) rather than blanking. Safe to nuke (next GET rebuilds it).
...
- **`pm:factor-universe`** / **`pm:factor-universe-progress`** / **`pm:factor-universe-names`** — the shadow factor model's weekly universe layer (`app/lib/factor-universe.ts`). `pm:factor-universe` holds winsorized per-GICS-sector metric distributions for ~540 S&P 500 + TSX 60 names; `-progress` is the chunk-resumable mid-build accumulator (cleared on finalize); `-names` holds every constituent's finished quant percentile + factor-group z's + Altman-style distress flag (the Universe Screen / idea-generation feed, `GET /api/factor-screen`). All three are pure regenerable caches — safe to nuke; the weekly rebuild (Sunday via `/api/cron/factor-universe`, paced by Apps Script pings) recreates them. Rebuild also forced when `LIST_VERSION` in `factor-constituents.ts` changes.
- **`pm:factor-scores`** — latest per-book-name shadow read-outs (quant percentile, judgment overlay, blend candidates), written nightly by `computeBookFactorScores()` (called from the backup cron). Regenerable cache — safe to nuke; force a rebuild via `/api/admin/factor-debug?book=1&run=1`. Served read-only by `GET /api/factor-scores` (Factor Lab + stock-page Factor Lens tile).
- **`pm:factor-history`** — **append-only** per-ticker daily log of the shadow read-outs, including point-in-time `price` and `s41` (41-pt adjusted) so Phase C IC validation is self-contained. Same date-guard invariant as `pm:portfolio-snapshots` (one row per ticker per day, today only, never overwrites). **BACKED UP** (deliberately NOT in the backup exclude list) — losing it means losing the validation evidence base. Read by `app/lib/factor-validation.ts` / `GET /api/factor-validation`. Do not clobber; schema changes must preserve prior rows.
...
- **`pm:street-takeaways`** — per-ticker log of parsed FactSet alert emails (`{ [TICKER]: StreetTakeaway[] }`, newest first). FOUR `kind`s share the store, each with its OWN retention window so they never evict one another: `takeaways` (Street Takeaways — the per-firm analyst roundup: PT changes with valuation basis + distilled argument, full-panel rating mix, average target, valuation vs the name's own 5-year history, consensus estimate revisions), `metrics` (StreetAccount Metrics Recap — reported results vs consensus, guidance vs prior guide, beat track record), `transcript` (Transcript Intelligence), and `news` (Top News Summaries flashes + manual forwards — a development BETWEEN prints: headline, the facts behind it, and every stated figure with its baseline). Earnings kinds keep `MAX_PER_KIND` (4) for `MAX_AGE_DAYS` (365); news keeps `MAX_NEWS_PER_TICKER` (10) for `NEWS_MAX_AGE_DAYS` (180) — news arrives on no schedule and goes stale faster. Ingested via the Gmail inbox — these are BODY-TEXT emails (no attachment), matched by sender (`FactSet_Alerts@factset.com`), by subject (`SA: …`, `Street Takeaways`, or the manual `News: <TICKER> …` / `FYI:` / `Note:` convention with the ticker in CAPS), or by the alert's own body footer (`isFactsetAlertBody`); parsed by a small Anthropic call in `app/lib/street-takeaways-parse.ts` against one of three schemas chosen by `detectTakeawayKind`, and scoped to Portfolio/Watchlist names (others skipped BEFORE any Anthropic call, so an off-book forward costs nothing). Deduped on (ticker, date, kind, event) — or (ticker, date, kind, normalized headline) for `news`, since several unrelated flashes can land on one day. Feeds the scoring prompt (catalysts / researchCoverage / historicalValuation), `app/lib/thesis-evidence.ts` (2 earnings alerts + 2 news flashes), the synthesis screen, and the stock page's FactSet Alerts tile. Read-only w.r.t. `pm:stocks` — it NEVER writes a score. Losing it costs the ingested analyst context until the next alerts arrive.
- **`pm:hedges`** — user-owned ledger of ACTUALLY-IMPLEMENTED hedge positions (strike, premium, tenor, entry spot, expiry, contracts, status active/closed). Ground truth for whether the book has protection on: the morning brief reads it and only recommends `hedgingCall.action = "HOLD"` when ≥1 active (non-expired, non-closed) position exists — with none on the books the call is coerced to ADD/SKIP (prompt guardrail + deterministic HOLD→SKIP backstop). Managed by `/api/kv/hedges` (GET/POST-replace) and the Brief Hedging tile's log/close UI; helpers in `app/lib/hedges.ts` (`isActiveHedge`, `describeHedge`, `realizedPnlUsd`). Never seed/clobber — GET returns `{hedges:[]}` on miss. Losing it means the brief reverts to assuming unhedged.
- **`pm:vol-anchor`** — cached long-horizon vol anchor for the hedging read (`getVolAnchor` in `app/lib/hedging.ts`): current ^VIX and ^VIX3M levels ranked as percentiles of their FULL Yahoo history (~35y / ~16y). The multi-regime guard against the hedging ledger's short collection window — "premium at the 20th percentile of our ledger" is only meaningful alongside "and VIX3M is at the Nth percentile of 16 years". Refreshed lazily every 24h. Pure cache — safe to nuke.
- **`pm:risk-analytics`** — cached output of `/api/risk-analytics` (book-level risk lens: covariance risk contributions, correlation clusters, beta-weighted look-through sector exposure, historical stress replays — computed from 1y of Yahoo daily closes for the Portfolio bucket). Pure regenerable cache (reads pm:stocks/pm:market READ-ONLY); recomputed on GET when older than 6h or `?refresh=1`. On compute failure the route serves the stale cache rather than blanking. Safe to nuke.
...
- **`pm:client-report-bullets-cache`** — hash-gated cache of the Anthropic-generated "Portfolio Highlights" talking points for the Client Report (`{ hash, result }`, `result = { bullets: string[], generatedAt }`). Distinct from `pm:client-report-analysis-cache`: those describe the CLIENT'S portfolio vs the model, these describe the MODEL alone (broad index exposure, diversification/breadth, blended MER, risk posture) and never name an individual position — they're written to be pasted into a client presentation. `/api/client-report-bullets` receives only asset-class / sector / geography / breadth-count / fee / risk aggregates — no tickers, no holding names, no client data. The Client Report page ALWAYS `await refetch()`s live report data before POSTing, so bullets can't be written off a stale snapshot; the generated copy is stored alongside a client-side fingerprint of those inputs in `pm:client-portfolio.presentationBullets`, and the UI flags "regenerate" when the live numbers drift from that fingerprint. `force: true` bypasses the cache. Safe to nuke (worst case: one Anthropic call on the next click).
