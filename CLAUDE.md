...
- **`pm:research-synthesis-cache`** — hash-gated cache of the Anthropic-generated cross-source synthesis at the top of the Research page (`{ hash, result, generatedAt }`). Hash covers the sorted ticker lists for all five sources (upticks, Fundstrat top/bottom, RBC focus, Alpha Picks) plus key brief fields (regime, three horizon views, bottomLine, hedging stance). When the inputs are unchanged, `/api/research-synthesis` returns the cached result with zero Anthropic spend. The synthesis page POSTs on mount with the current state to hydrate from cache; the user's "Generate" / "Refresh" button re-POSTs the same payload (still cache-gated); only "Force re-generate" bypasses. Safe to nuke (worst case: one re-run next time the user opens Research).
