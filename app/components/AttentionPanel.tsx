...
import type { Alert } from "@/app/lib/alerts";
...
export function AttentionPanel() {
  ...
  useEffect(() => {
    ...
    fetch("/api/alerts", { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => alive && setAlerts(Array.isArray(j?.alerts) ? j.alerts : []))
  ...
  return (
    <section className="rounded-card border border-warn-border bg-warn-soft/40 p-4 shadow-sm">
