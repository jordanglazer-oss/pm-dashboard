...
${rows.join("\n")}`;
    ...
    const transitionBlock = regimeTransition
      ? `\n\nREGIME-TRANSITION GAUGE (forward — how close the CURRENT regime is to flipping; use in tacticalView / cyclicalView and bottomLine to position AHEAD of a flip, not after it):\n- ${regimeTransition.basedOnRegime}, leaning ${regimeTransition.leaning}; transition likelihood ${regimeTransition.likelihood} (heuristic ${regimeTransition.score}/90); ${regimeTransition.boundaryGap} signal(s) from a label change.${
