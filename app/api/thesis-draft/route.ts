...
- CADENCE TEST — the metric must be reported EVERY QUARTER, not merely disclosed sometimes. Subscription member counts, cumulative user milestones, driver/courier counts and similar are announced at milestones or investor days, so most quarters cannot answer them and the condition sits permanently unverifiable. Use the quarterly line the metric DRIVES instead: not "Uber One membership must not decline from 50M" but "Delivery gross bookings growth must not fall below X% Y/Y" or "membership-driven revenue must not decline sequentially".
- DISCLOSURE TEST — the most common failure. Only use a metric the company ACTUALLY PUBLISHES at that granularity. Prefer, in order: (a) a figure that appears verbatim in the ingested evidence above, (b) a standard reported line (segment revenue, total revenue, EPS, gross/operating margin, guidance), (c) nothing — replace the idea. Do NOT invent segment-level detail that is not broken out: Alphabet reports total RPO, not "GCP backlog"; most issuers do not disclose per-product margin, per-customer revenue, or unit volumes. A condition against an undisclosed metric can never be verified and permanently reads NO DATA, which is worse than not having it — it looks watched but nothing is watching.
