...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch the page with a fuller browser-like header set. Many fund
    // issuer sites (Schwab, BlackRock, Fidelity) sit behind Cloudflare
    // or Akamai bot protection that rejects requests with sparse
    // headers. The fuller set below gets past simple UA-only checks
    // (Sec-Fetch-* headers tell the server this looks like a top-level
    // navigation from a real browser). Won't bypass full bot
    // protection that requires JS execution — for those sites the
    // user should use Morningstar instead, which serves to scrapers.
    const res = await fetch(url, {
      cache: "no-store",
      // 10s timeout so a hung connection doesn't block the request.
    ...
    if (!res.ok) {
      // Distinguish bot-block (403/406/429) from genuine 404 so the
      // UI can suggest Morningstar as a fallback for blocked sites.
      const blockedByBot = [403, 406, 429].includes(res.status);
      const hint = blockedByBot
        ? " (this site is blocking server-side requests — try Morningstar's fund page instead, which usually allows scraping)"
        : res.status === 404
          ? " (URL not found — check the link or try Morningstar's fund page)"
          : "";
      return NextResponse.json(
        { error: `Failed to fetch URL: ${res.status} ${res.statusText}${hint}` },
        { status: 502 }
      );
    }
