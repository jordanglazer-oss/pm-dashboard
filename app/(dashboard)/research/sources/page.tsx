...
export default function ResearchPage() {
  ...
  /** Deep link from the Ranked tab: /research/sources#<railKey> opens that
   *  pane once the blob has loaded (a native anchor can't — the page renders
   *  nothing until `loaded`). */
  const hashHandled = useRef(false);
  useEffect(() => {
    if (!loaded || hashHandled.current) return;
    const hash = typeof window !== "undefined" ? window.location.hash.replace(/^#/, "") : "";
    if (!hash || !ALL_RAIL_KEYS.includes(hash)) return;
    hashHandled.current = true;
    openSourcePane(hash);
  }, [loaded, openSourcePane]);
